...
public class ConfigurationManager {
		...
		if (nodes != null && nodes.size() > 0) {
			...
			for (Element ele : nodes) {
				...
				for(Element e:results) {
					...
					result.setLocaltion(e.getStringValue());
					ac.getResultMap().put(result.getName(), result);
	...
	private static Document getDocument() {
		...
		// 加载配置文件
		InputStream is = ConfigurationManager.class.getClassLoader().getResourceAsStream("/bluedotcontrol.xml");
		Document doc = null;
		try {
			doc = reader.read(is);
